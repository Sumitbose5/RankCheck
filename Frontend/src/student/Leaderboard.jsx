...
export const Leaderboard = () => {
  ...
  useEffect(() => {
    setHeaderText("Leaderboard");  // Update header when mounted
  }, [setHeaderText]);


  const { data, isPending, isError, error } = useQuery({
    queryKey: ['rankPages', pageNumber, semester],
    queryFn: () => fetchRank(pageNumber, class_name, semester),  // send the class_name 
  ...
  if (isError) return <p className="text-red-400">{error}</p>;
      ...
      <div className="w-full max-w-4xl bg-gray-800 rounded-xl shadow-2xl p-4 sm:p-6 border border-gray-700">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2  items-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-200 tracking-wide">
              🏆 Leaderboard
            </h2>
            <span className="text-xs sm:text-sm sm:hidden block rounded-lg text-gray-100 font-semibold">
              Semester
            </span>
          </div>
        ...
        <div className="flex justify-center items-center gap-3 sm:gap-4 mt-4 sm:mt-6">
          <button
            className={`px-3 sm:px-4 py-1 sm:py-2 rounded-lg text-xs sm:text-sm transition-all ${pageNumber === 1 ? "bg-gray-600 cursor-not-allowed opacity-60" : "bg-indigo-600 hover:bg-indigo-500 cursor-pointer"}`}
          ...
          <button
            className={`px-3 sm:px-4 py-1 sm:py-2 rounded-lg text-xs sm:text-sm transition-all ${pageNumber === 6 ? "bg-gray-600 cursor-not-allowed opacity-60" : "bg-indigo-600 hover:bg-indigo-500 cursor-pointer"}`}
